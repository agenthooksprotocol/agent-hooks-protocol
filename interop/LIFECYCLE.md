...
Raw octets go to readiness `uploadEndpoint` using independently configured upload credentials. ONLY subscription `body` is authorized (others 403). Verify size/hash (400). Same subscription+ref changed bytes 409. New ref/exact retry 204; store immutable bytes BEFORE success.
