...
s['steps']=[upload(ref,'original bytes'),upload(ref,'original bytes'),upload(ref,'changed bytes'),upload('immutable-v2','changed bytes'),upload(ref,'original bytes','metadata')]+start()+finish()+[op('observe',key='a',subscription='body',items=[body]),op('observe',key='a',subscription='metadata',items=[gap])]; s['expected']=actual(published=[rid(s)],states={rid(s):state()},uploadStatuses=[204,204,409,204,403],observations=[{'eventId':rid(s),'subscription':x,'input':{'value':'settled'}} for x in ['body','metadata']]); scenarios.append(s)
...
s['steps']+=start()+finish()+[op('observe',key='a',subscription='body',items=items)]; s['expected']=actual(published=[rid(s)],states={rid(s):state()},uploadStatuses=[204]*3,observations=[{'eventId':rid(s),'subscription':'body','input':{'value':'settled'}}]); scenarios.append(s)
...
s['expected']=actual(published=[rid(s)],states={rid(s):state()},uploadStatuses=[204,204],observations=[{'eventId':rid(s),'subscription':'body','input':{'value':'settled'}}]); scenarios.append(s)
s=scenario('intercept-content-upload-before-send',EFFECTS)
s['requests']['a']['params']['subscriptionId']='body'
...
s['expected']=actual(published=[rid(s)],states={rid(s):state()},uploadStatuses=[204]); scenarios.append(s)
