...
from content_upload import Receiver, serve, reference, upload, deliver, selected_item, encoded
...
class ContentBindingTests(TestCase):
    def setUp(self):
        ...
        self.message = {'jsonrpc':'2.0','method':'hooks/observe','params':{
            'protocolVersion':'draft','event':event,'subscriptionId':'body'}}
    ...
    def headers(self, body=b''):
        return [('Content-Type','application/octet-stream'),('Content-Length',str(len(body))),
                ('Authorization','Bearer upload'),('AHP-Subscription',encoded('body')),
                ('AHP-Content-Ref',encoded('ref')),('AHP-Content-SHA256',reference('ref',body)['sha256'])]
    ...
    def test_binary_upload_confirmed_before_actual_event(self):
        ...
        self.assertEqual(self.receiver.contents[('body','ref')],body)
        self.assertEqual([x[0] for x in self.receiver.trace],['upload','event'])
        self.assertEqual(self.receiver.events,[self.message])
    ...
    def test_empty_body_is_not_missing(self):
        ...
        self.assertEqual(self.receiver.contents[('body','ref')],b'')
    ...
    def test_anonymous_upload_does_not_inherit_event_auth(self):
        ...
        with self.assertRaises(HTTPError) as error:
            deliver(self.message,'body',self.config,{'ref':b'x'},self.endpoint+'/observe','event')
        self.assertEqual(error.exception.code,403); self.assertEqual(self.receiver.contents,{})

    def test_explicit_anonymous_authorization(self):
        self.config.pop('auth'); upload(self.config,'anonymous',reference('ref',b'x'),b'x')
        self.assertEqual(self.receiver.contents[('anonymous','ref')],b'x')

    def test_immutable_retry_and_changed_bytes(self):
        for _ in range(2): upload(self.config,'body',reference('ref',b'a'),b'a')
        with self.assertRaises(HTTPError) as error: upload(self.config,'body',reference('ref',b'b'),b'b')
        self.assertEqual(error.exception.code,409); self.assertEqual(self.receiver.contents[('body','ref')],b'a')
    ...
    def test_declared_subscription_cannot_override_authorized_scope(self):
        self.attach(b'x'); self.message['params']['subscriptionId']='anonymous'
        with self.assertRaises(HTTPError) as error:
            deliver(self.message,'body',self.config,{'ref':b'x'},self.endpoint+'/observe','event')
        self.assertEqual(error.exception.code,403); self.assertEqual(self.receiver.events,[])
    ...
    def test_upload_auth_does_not_authorize_event(self):
        ...
        self.assertEqual(self.receiver.contents[('body','ref')],b'x')
