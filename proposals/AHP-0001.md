...
> This tracked migration preserves the complete substantive content of the source proposal as fetched on 2026-08-26. Notion-only metadata, discussion identifiers, user identifiers, and private source links were omitted. The canonical normative Working Draft is [`../spec/working-draft.md`](../spec/working-draft.md). Differences must be proposed publicly; this document does not silently resolve the open questions in Section 23.
...
```mermaid
flowchart LR
    A[Existing harness] --> B[Compatibility adapter]
    B -->|AHP| C[AHP backend]
    D[Conforming harness] -->|AHP| C
```
## 2. Scope
### 2.1 Goals
AHP v0.1 aims to:
...
### 5.1 Base Protocol profile
Every conforming AHP v0.1 implementation MUST:
- Correctly encode every JSON-RPC message it sends and decode every JSON-RPC message it receives, as defined in Section 6.
...
### 5.3 Lifecycle Observation profile
The Lifecycle Observation profile is OPTIONAL. A conforming implementation of this profile supports `hooks/observe` and one or more events defined in Section 8.
...
## 7. Common message envelope
### 7.1 Intercept request
```json
               {
  "jsonrpc": "2.0",
...
### 7.2 Intercept response with no effect
```json
{
  "jsonrpc": "2.0",
  "id": "evt_01JQ8Z2Y6YR0H8N7Q2M3X4V5W6",
...
### 7.3 Intercept response denying the operation
```json
{
  "jsonrpc": "2.0",
  "id": "evt_01JQ8Z2Y6YR0H8N7Q2M3X4V5W6",
...
### 12.3 Overlapping subscriptions
...
## 14. Duplicate delivery and retries
...
#### 16.1.3 Process-per-event lifecycle
...
### 17.6 Native harness configuration
...
### 19.2 Tool Interception harness tests
...
### 19.3 Tool Interception backend tests
...
### 19.4 Lifecycle Observation tests
...
## 20. Compatibility-adapter requirements
...
### 21.2 Why capabilities if v0.1 has one effect?
...
### 21.5 Why ordered serial composition?
...
## 22. Relationship to adjacent standards
### 22.1 OpenTelemetry
...
### 22.3 Prior-art principles adopted by v0.1
